...
class DANSE(nn.Module):
    ...
    def __init__(self, n_states, n_obs, mu_w, C_w, H, mu_x0, C_x0, batch_size, rnn_type, rnn_params_dict, device='cpu'):
        ...
        # self.H = torch.nn.Parameter(torch.eye(n_obs, n_states), requires_grad=True).to(self.device)
    ...
    '''
    def compute_logpdf_Gaussian(self, Y, mu_yt_current, L_yt_current):
        
        _, T, _ = Y.shape 
        logprob = - 0.5 * self.n_obs * T * math.log(math.pi*2) - 0.5 * torch.logdet(L_yt_current).sum(1) \
            - 0.5 * torch.einsum('nti,nti->nt',
            (Y - mu_yt_current),
            torch.einsum('ntij,ntj->nti',torch.inverse(L_yt_current), (Y - mu_yt_current))).sum(1)
    ...
    def forward(self, Yi_batch, Cw_batch):
        ...
        # print(self.H.device, self.mu_xt_yt_prev.device, self.mu_w.device)
        mu_yt_current = torch.einsum('ij,ntj->nti', self.H, mu_xt_yt_prev) + self.mu_w.squeeze(-1)
        L_yt_current = self.H @ L_xt_yt_prev @ torch.transpose(self.H, 0, 1) + Cw_batch_seq  # + self.C_w

        _, T, _ = Yi_batch.shape
        logprob = - 0.5 * self.n_obs * T * math.log(math.pi*2) - 0.5 * torch.logdet(L_yt_current).sum(1) \
            - 0.5 * torch.einsum('nti,nti->nt',
            (Yi_batch - mu_yt_current),
            torch.einsum('ntij,ntj->nti',torch.inverse(L_yt_current), (Yi_batch - mu_yt_current))).sum(1)
        logprob_batch = logprob / (Yi_batch.shape[1] * Yi_batch.shape[2]) # Per dim. and per sequence length
