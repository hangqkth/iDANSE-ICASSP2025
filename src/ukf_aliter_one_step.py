    ...
    def run_mb_filter(self, X, Y, U=None):
        ...
        for i in range(0, N):
            ...
            for k in range(0, Ty):
                ...
                self.ukf.predict()
                traj_estimated_predicted[i,k,:] = torch.from_numpy(self.ukf.x)
                Pk_estimated_predicted[i,k,:,:] = torch.from_numpy(self.ukf.P)
                self.ukf.update(Y[i,k,:].numpy())       
                traj_estimated_filtered[i,k,:] = torch.from_numpy(self.ukf.x)
                Pk_estimated_filtered[i,k,:,:] = torch.from_numpy(self.ukf.P)
