...
from src.idanse import IDANSE, train_idanse  # , test_danse
...
def main():
    ...
    parser.add_argument("--bidirectional", help="use bidrectional model", type=str)
    ...
    parser.add_argument("--model_file_saved",
    ...
    data_string, n_states, n_obs, _, T, N_samples, sigma_e2_dB, smnr_dB = parse(
    ...
    estimator_options[
        'C_w'] = ssm_model.Cw  # Get the covariance matrix of the measurement noise from the model information
    estimator_options['H'] = get_H_DANSE(type_=dataset_type, n_states=n_states,
                                         n_obs=n_obs)  # Get the sensing matrix from the model info
    ...
    # print(params)
    tr_log_file_name = "danseq_training.log"
    ...
    if mode.lower() == "train":
        model_danse = IDANSE(**estimator_options)
        tr_verbose = True

        # Starting model training
        tr_losses, val_losses, val_mse_losses, epochs_at_k, _, _, _ = train_idanse(
            model=model_danse,
        ...
        losses_model["val_mse_losses"] = val_mse_losses
        losses_model["epochs_at_k"] = epochs_at_k

        with open(os.path.join(os.path.join(logfile_path, main_exp_name),
                               'idanse_{}_losses_eps{}.json'.format(estimator_options['rnn_type'],
                                                                   estimator_options['rnn_params_dict'][model_type][
                                                                       'num_epochs'])), 'w') as f:
